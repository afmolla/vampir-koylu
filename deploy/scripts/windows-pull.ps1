...
if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
    ...
    if (Test-Path $gitExe) { $git = $gitExe } else {
        Write-Host "Git yok. ZIP indir: https://github.com/afmolla/vampir-koylu/archive/refs/heads/main.zip" -ForegroundColor Red
        exit 1
    }
}

Write-Host ">> git pull (repo: afmolla/vampir-koylu, klasor: $Root)" -ForegroundColor Cyan
